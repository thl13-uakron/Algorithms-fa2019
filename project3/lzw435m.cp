...
const int BITS = 9; // size of each code
...
void compress_file(std::string infileName, std::string outfileName) {
    ...
    int codeLength = BITS;
    // number of codes of this lengthrecorded so far
    int codesWritten = 0;
    ...
    while (bytesRead < numBytes) {
      ...
      if (mappings.find(newCharSequence) == mappings.end()) {
        ...
        // std::cout << mappings[charSequence] << " " << charSequence << " " << codeLength << std::endl;
        // update bit length of codes if needed
        if (++codesWritten == 1 << (codeLength - 1)) {
          ++codeLength;
          codesWritten = 0;
        }
...
void decompress_file(std::string infileName, std::string outfileName) {
    ...
    // number of bits per code
    int codeLength = BITS;
    // number of codes of this length read
    int codesRead = 0;
    ...
    while (bytesRead < numBytes) {
      ...
      // std::cout << nextCode << " " << nextWord << " " << codeLength << std::endl;
    }
  ...
  */
  ...
  if (argc < 3) {
    ...
    if (strcmp(argv[1], "c") == 0 || strcmp(argv[1], "C") == 0) {
      outfileName = infileName + ".lzwm";
      std::cout << "Compressing file " << infileName << std::endl;
      compress_file(infileName, outfileName);
    }
    else {
      outfileName = infileName;
      outfileName.replace(outfileName.find(".lzwm"), 5, "");
      // handle existing file extensions
      if (outfileName.find(".") == std::string::npos) outfileName += "2m";
      else outfileName.insert(outfileName.find("."), "2m");
